...
class TestInteger(BaseTest):
    ...
    def test_generate(self, mocker, fields_to_exclude):
        ...
        lower_bound = spec.minimum or (spec.exclusive_minimum + 1 if spec.exclusive_minimum else None) or -sys.maxsize - 1
        upper_bound = spec.maximum or (spec.exclusive_maximum - 1 if spec.exclusive_maximum else None) or sys.maxsize
